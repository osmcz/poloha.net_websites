...
if (isset($_REQUEST['latlng'])) {
	...
	select so.kod,nb.datum,nb.user_nick,so.item_timestamp,nb.duvod,zv.nazev,ruian.adresa_budovy(so.kod),
	    duvod.popis,nb.poznamka,nb.hlasit_cuzk,so.hranice is not NULL as ma_geom,so.kod IS NOT NULL AS existuje
...
if (isset($_REQUEST['kod'])) {
    ...
    $RESULT=pg_query($CONNECT,"
	select nb.kod,nb.datum,nb.user_nick,so.item_timestamp,nb.duvod,zv.nazev,ruian.adresa_budovy(so.kod),
	    duvod.popis,nb.poznamka,nb.hlasit_cuzk,so.hranice is not NULL as ma_geom,so.kod IS NOT NULL AS existuje
	from ruian.rn_stavebni_objekt so
	left join osmtables.neplatne_budovy nb on so.kod=nb.kod
	left join osmtables.neplatne_budovy_duvod duvod on nb.duvod=duvod.id
	left join osmtables.zpusob_vyuziti_objektu zv on so.zpusob_vyuziti_kod=zv.kod
	where nb.kod=".$kod." and (not so.deleted or so.kod is NULL)
	limit 1
    ");
    if (pg_num_rows($RESULT) != 1) {
    $RESULT=pg_query($CONNECT,"
	select nb.kod,nb.datum,nb.user_nick,so.item_timestamp,nb.duvod,NULL::text as nazev,NULL::text as adresa_budovy,
	    duvod.popis,nb.poznamka,nb.hlasit_cuzk,so.hranice is not NULL as ma_geom,so.kod IS NOT NULL AS existuje
	from osmtables.neplatne_budovy nb
	left join ruian.rn_stavebni_objekt so on so.kod=nb.kod
	left join osmtables.neplatne_budovy_duvod duvod on nb.duvod=duvod.id
	left join osmtables.zpusob_vyuziti_objektu zv on so.zpusob_vyuziti_kod=zv.kod
	where nb.kod=".$kod." and (not so.deleted or so.kod is NULL)
	limit 1
...
if ($queryran) {
    ...
    if ((pg_result($RESULT,0,"item_timestamp") > pg_result($RESULT,0,"datum")) and pg_result($RESULT,0,"ma_geom") == "t" and pg_result($RESULT,0,"existuje") =="t") echo("<img src=\"neplatne-budovy/task-attention.png\"><b>Po zadání sem byla budova v RÚIAN aktualizována</b>");
    if ((pg_result($RESULT,0,"item_timestamp") > pg_result($RESULT,0,"datum")) and pg_result($RESULT,0,"ma_geom") != "t" and pg_result($RESULT,0,"existuje") == "t") echo("<img src=\"neplatne-budovy/face-sad.png\"><b>Po zadání sem byla z budovy v RÚIAN odstraněna její geometrie</b>");
    if (pg_result($RESULT,0,"existuje") != "t") echo("<img src=\"neplatne-budovy/task-reject.png\"><b>Budova z RÚIAN zmizela</b>");
